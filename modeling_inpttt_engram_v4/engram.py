...
class NgramHashMapping:
    ...
    def _get_ngram_hashes(
        ...
        for ngram_idx, n in enumerate(self.ngram_size_list):
            tokens = base_shifts[:n]
            mix = (tokens[0] * multipliers[0])
            for k in range(1, n):
                mix = np.bitwise_xor(mix, tokens[k] * multipliers[k])
            num_heads_for_this_ngram = self.n_head_per_ngram
            head_vocab_sizes = self.vocab_size_across_layers[layer_id][ngram_idx]
...
class EngramEmbedding(nn.Module):
    def __init__(self, config, layer_id):
        ...
        # Convert dict to EngramConfig dataclass
        engram_config_dict = config.engram_config[layer_id]
        if isinstance(engram_config_dict, dict):
            engram_config = EngramConfig(**engram_config_dict)
        else:
            engram_config = engram_config_dict
        self.engram_config = engram_config

        self.hash_mapping = NgramHashMapping(
            engram_vocab_size=engram_config.engram_vocab_size,
            ngram_size_list=engram_config.ngram_size_list,
        ...
        engram_hidden_size = len(engram_config.ngram_size_list) * engram_config.n_embed_per_ngram
        self.proj = nn.Linear(engram_hidden_size, config.hidden_size)

    def forward(self, input_ids, past_input_ids=None):
        """
        input_ids: [B, L]
        past_input_ids: [B, past_L], only the last (max_ngram_size-1) tokens needed
        Returns: [B, L, hidden_size]
        """
        L = input_ids.shape[1]
        max_ngram = max(self.engram_config.ngram_size_list)

        if past_input_ids is not None:
            # Only keep the last (max_ngram - 1) tokens from past
            needed_past_len = max_ngram - 1
            if past_input_ids.shape[1] > needed_past_len:
                past_input_ids = past_input_ids[:, -needed_past_len:]
            full_input_ids = torch.cat([past_input_ids, input_ids], dim=1)
        else:
            full_input_ids = input_ids

        hash_input_ids = torch.from_numpy(
            self.hash_mapping.hash(full_input_ids.cpu().numpy())[self.layer_id]
        )
        if input_ids.is_cuda:
            hash_input_ids = hash_input_ids.to(input_ids.device)

        embeddings = self.multi_head_embedding(hash_input_ids).flatten(start_dim=-2)
        return self.proj(embeddings[:, -L:])
...
class Engram(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.short_conv = ShortConv(
            ...
            dilation=max(engram_config.ngram_size_list),
            hc_mult=self.hc_mult,
        )
        engram_hidden_size = len(engram_config.ngram_size_list) * engram_config.n_embed_per_ngram
        self.value_proj = nn.Linear(engram_hidden_size, self.hidden_size)
        self.key_projs = nn.ModuleList(
            [nn.Linear(engram_hidden_size, self.hidden_size) for _ in range(self.hc_mult)]
        )
