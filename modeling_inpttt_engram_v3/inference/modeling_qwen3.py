...
from .engram import EngramEmbedding
...
class TTTDynamicCache(DynamicCache):
    def __init__(self, ddp_cache_data=None, config=None) -> None:
        ...
        if config is not None and hasattr(config, 'engram_config') and config.engram_config is not None:
            max_ngram_size = 1
            for layer_idx, engram_config in config.engram_config.items():
                max_ngram_size = max(max_ngram_size, max(engram_config.ngram_size_list))
...
class Qwen3MLP(nn.Module):
    def __init__(self, config, layer_idx: Optional[int] = None):  # TTT: added layer_idx
        ...
        # Determine inter_hidden_size based on engram config
        # If this layer is >= the first engram layer, use the most recent engram layer's hidden size
        engram_layers = sorted(config.engram_config.keys()) if config.engram_config else []
        if engram_layers and layer_idx >= engram_layers[0]:
            # Find the largest engram layer <= current layer_idx
            effective_engram_layer = max([l for l in engram_layers if l <= layer_idx])
            engram_cfg = config.engram_config[effective_engram_layer]
            inter_hidden_size = len(engram_cfg["ngram_size_list"]) * engram_cfg["n_embed_per_ngram"]
        else:
            inter_hidden_size = self.hidden_size
        if getattr(config, "ttt_mode", False) and self.layer_idx in getattr(config, "ttt_layers", []):
            ...
            if getattr(config, "ttt_proj", True):
                self.ttt_proj = nn.Linear(inter_hidden_size, self.hidden_size, bias=False)
            else:
                self.ttt_proj = None
            self.ttt_lr = getattr(config, "ttt_lr", 0.3)
            self.ttt_conv = nn.Conv1d(
                inter_hidden_size, inter_hidden_size, kernel_size=5, padding=2,
                groups=inter_hidden_size, bias=False,
            )
        ...
        for i, current_y, current_t, current_h in zip(range(chunk_num), y[0], t_conv[0], h_padded[0]):
            ...
            if seq_len % self.ttt_chunk == 0 or i != chunk_num - 1:
                if self.ttt_proj is not None:
                    dw = (
                        contract("c h, c d, e d -> e h", current_h, current_t, self.ttt_proj.weight) * self.ttt_lr
                    )
                else:
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        ...
        if layer_idx in engram_config:
            self.engram_embedding = EngramEmbedding(config, layer_id=layer_idx)
        else:
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Engram: get past_input_ids from cache
        engram_past_input_ids = None
        if isinstance(past_key_values, TTTDynamicCache):
            engram_past_input_ids = past_key_values.get_past_input_ids()

        engram_target: Optional[torch.Tensor] = None
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            if decoder_layer.engram_embedding is not None and input_ids is not None:
                engram_target = decoder_layer.engram_embedding(input_ids, past_input_ids=engram_past_input_ids)
            ttt_target = self._resolve_ttt_target_states(decoder_layer, inputs_embeds, engram_target)
