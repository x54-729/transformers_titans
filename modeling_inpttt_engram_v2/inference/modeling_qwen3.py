...
class TTTDynamicCache(DynamicCache):
    def __init__(self, ddp_cache_data=None, config=None) -> None:
        ...
        if config is not None and hasattr(config, 'engram_config') and config.engram_config is not None:
            self.max_context_length = config.engram_config.max_ngram_size - 1
...
class Qwen3MLP(nn.Module):
    def __init__(self, config, layer_idx: Optional[int] = None):  # TTT: added layer_idx
        ...
        # is engram
        engram_hidden_size = (config.engram_config.max_ngram_size - 1) * config.engram_config.n_embed_per_ngram
        min_engram_idx = min(config.engram_layer_idx)
        if layer_idx >= min_engram_idx:
            inter_hidden_size = engram_hidden_size
        else:
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        ...
        # Engram
        engram_layer_idx = getattr(config, "engram_layer_idx", [])
        if layer_idx in engram_layer_idx and getattr(config, "engram_config", None) is not None:
