...
class HybridBlock(nn.Module):
    """
    Hybrid block that can optionally include Engram before Attention/LaCT.
    Supports three modes:
    1. Engram + Qwen3Attention
    2. Engram + LaCT
    3. No Engram (just Qwen3Attention or LaCT)
    """
    ...
    def __init__(
        ...
        if self.use_engram:
            ...
            self.hc_mult = config.engram_config.hc_mult
        else:
            self.engram = None
            self.hc_mult = 1

        # Attention norm and module
    ...
    def forward(
        ...
        **kwargs: Unpack[Any],
        ...
        # Engram processing (if enabled)
        if self.use_engram and self.engram is not None:
            # Expand hidden_states for hyper-connection: [B, L, D] -> [B, L, HC_MULT, D]
            hidden_states_hc = hidden_states.unsqueeze(2).expand(-1, -1, self.hc_mult, -1)

            # Apply Engram
            engram_output = self.engram(hidden_states_hc, input_ids.cpu().numpy())

            # Merge back: [B, L, HC_MULT, D] -> [B, L, D]
            # Use mean or first slice to merge
            engram_output = engram_output.mean(dim=2)  # Or use engram_output[:, :, 0, :]
            # engram_output = engram_output[:, :, 0, :]

            # Residual connection after Engram
        ...
        # Both LaCT and Qwen3Attention now have the same interface
...
class EngramLaCTPreTrainedModel(PreTrainedModel):

    config_class = EngramLaCTConfig
...
class EngramLaCTModel(EngramLaCTPreTrainedModel):

    def __init__(self, config: EngramLaCTConfig) -> LaCTModel:
        ...
        engram_layer_set = set(config.engram_layer_idx)
        lact_layer_set = set(config.lact_layer_idx)

        for layer_idx in range(config.num_hidden_layers):
            use_engram = layer_idx in engram_layer_set
            use_lact = layer_idx in lact_layer_set

            # Use HybridBlock to handle all combinations
            layer = HybridBlock(
                config=config,
                layer_idx=layer_idx,
...
class EngramLaCTForCausalLM(EngramLaCTPreTrainedModel, GenerationMixin):

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = EngramLaCTModel(config)
        self.vocab_size = config.vocab_size
