...
def resume_model(work_dir, model_class, model_config, tokenizer_path):
    ...
    dist.barrier()
    model = AutoModelForCausalLM.from_pretrained(model_dir, trust_remote_code=True, attn_implementation=config.ATTENTION).cuda()
...
#     on_trace_ready=torch.profiler.tensorboard_trace_handler(f'{config.WORK_DIR}/profiler_rank{RANK}'),
#     record_shapes=True,
#     profile_memory=True,
#     with_stack=True
# )
