...
class Qwen3MLP(nn.Module):
    ...
    def forward(self, x, t: Optional[torch.Tensor] = None):  # TTT: added t param
        ...
        if self.ttt_proj is not None:
            # ttt_proj.weight shape is [out_features, in_features] = [hidden_size, ttt_hidden_size or hidden_size]
            # einsum expects [in_features, out_features], so we transpose
            d_down_proj = contract(
                "b t c h, b t c d, d e -> b t e h",
                h_padded[:, :-1], t[:, :-1], self.ttt_proj.weight.T,
            )
        else:
            d_down_proj = contract(
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Engram: process engram before attention if this layer has engram
        ...
        if self.has_engram and self.engram is not None:
            ...
            engram_output, engram_embedding = self.engram(hidden_states_expanded, input_ids)
        ...
        hidden_states = self.mlp(hidden_states, t=mlp_target)
        hidden_states = residual + hidden_states
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Engram: track engram_target across layers
        engram_target = None
