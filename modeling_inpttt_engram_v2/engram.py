...
class MultiHeadEmbedding(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        # In FSDP2, buffers may become DTensors, but embedding indices must be regular tensors
        # Convert offsets to regular tensor if needed
        offsets = self.offsets
        if DTENSOR_AVAILABLE and isinstance(offsets, DTensor):
